...
Interaktivni dokazivač koji se zasniva na prirodnoj dedukciji treba na početku
da prihvati sa ulaza formulu prvog reda i da zatim očekuje od korisnika pravila
prirodne dedukcije koja primenjuje kako ih korisnik navodi.

Osnovna ideja na kojoj dokazivači rade je da se pravila prirodne dedukcije
(slika \ref{fig:inf-rules}) primenjuju u suprotnom smeru u odnosu na to kako su
navedena.
...
\begin{serbianalgorithm}[H]
    ...
    \label{alg:varijacije_u_broj}
    \textbf{Ulaz:} Formula prvog reda $F$ \\
    \textbf{Izlaz:} Program se završava kada se formula $F$ dokaže u prirodnoj dedukciji
    \begin{algorithmic}[1]
        \State $s \gets \{ \vdash F \}$
        \While{$s$ nije prazan}
            \State $p \gets$ pravilo sa standardnog ulaza
